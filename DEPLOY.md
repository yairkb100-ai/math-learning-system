...
קבצים ב-**Cloudflare R2**, ורייט-לימיטר ב-**Upstash Redis** (אינטגרציית Vercel Marketplace).
...
לפונקציה), `DATABASE_URL`/Postgres מבוסס env, `REDIS_URL` ל-rate-limiter, `CLOUDFLARE_R2_*` לכל ההעלאות.
...
4. **Settings → Environment Variables** (Production):
   ...
   | `CLOUDFLARE_R2_ACCOUNT_ID` / `CLOUDFLARE_R2_ACCESS_KEY_ID` / `CLOUDFLARE_R2_SECRET_ACCESS_KEY` / `CLOUDFLARE_R2_BUCKET` / `CLOUDFLARE_R2_PUBLIC_URL` | מ-Cloudflare dashboard → R2 |
...
2. הרצת סקריפט הגירת קבצים (`backend/migrate_bunny_to_r2.py`, או `migrate_uploads_to_bunny.py`
   הישן להעלאות מקומיות) אם יש קבצים ישנים שלא עברו.
...
- הקורסים מופיעים.
- העלאת קובץ (גם שיעורי בית/הודעות, לא רק חומר קורס) עוברת ל-R2 ונשמרת.
...
- להוספת קורס: מוסיפים `courses/<slug>.json`, commit+push — ה-workflow ב-GitHub Actions מריץ
  `seed.py` מול Neon.
- כל ההעלאות (resource/homework/message) הולכות ל-R2 — אין דיסק מקומי פרסיסטנטי בסביבת
  serverless.
