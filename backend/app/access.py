...
def free_chapter_quota(total_chapters: int) -> int:
    ...
    עיגול כלפי מטה ולא לקרוב: ההבטחה היא "עד 42%", ועיגול לקרוב היה חורג ממנה
    (קורס בן 6 פרקים היה נותן 3 — 50%). המינימום הוא פרק אחד, אחרת קורס קצר
    (עד 4 פרקים) היה נפתח באפס פרקים ולא הייתה שום טעימה בכלל.
    """
    if total_chapters <= 0:
        return 0
    return max(1, int(total_chapters * FREE_CONTENT_RATIO))
...
def asset_is_unlocked(
    ...
    שדורש רק התחברות — כלומר אפשר היה להוריד את הסרטונים של 58% הנעולים בלי
    לפתוח את הפרק בכלל.
